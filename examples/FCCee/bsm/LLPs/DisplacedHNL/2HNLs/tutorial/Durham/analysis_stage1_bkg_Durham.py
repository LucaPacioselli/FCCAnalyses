...
outputDir = "/eos/user/l/lpaciose/bkg2"
...
########################################## Trying to use DurhamN2 reconstructed jets
            ...
            # run jet clustering with all reconstructed particles. Durham algo ("clustering_ee_kt"), exclusive clustering N=2, E-scheme 
            df = df.Define( "JetDurhamN2",  "JetClustering::clustering_ee_kt(2, 0, 1, 0)(pseudo_jets)" ) #provo a richiedere 0 jet ricostruiti
            df = df.Define("Jet_Durham",  "JetClusteringUtils::get_pseudoJets( JetDurhamN2 )")

            # access the jets kinematics :
            df = df.Define("RecoJet_e",      "JetClusteringUtils::get_e( Jet_Durham )")
            df = df.Define("RecoJet_p",      "JetClusteringUtils::get_p( Jet_Durham )") #momentum p
            df = df.Define("RecoJet_pt",      "JetClusteringUtils::get_pt( Jet_Durham )") #transverse momentum pt
            df = df.Define("RecoJet_px",  "JetClusteringUtils::get_px( Jet_Durham )")
            df = df.Define("RecoJet_py",  "JetClusteringUtils::get_py( Jet_Durham )")
            df = df.Define("RecoJet_pz",  "JetClusteringUtils::get_pz( Jet_Durham )")
		    
            df = df.Define("RecoJet_eta",     "JetClusteringUtils::get_eta(Jet_Durham )") #pseudorapidity eta
            df = df.Define("RecoJet_theta",   "JetClusteringUtils::get_theta(Jet_Durham )")
		    
            df = df.Define("RecoJet_phi",     "JetClusteringUtils::get_phi(Jet_Durham )") #polar angle in the transverse plane phi
            #df = df.Define("RecoJet_charge",  "JetClusteringUtils::get_charge(Jet_Durham )")
            #df = df.Define("RecoJetTrack_absD0", "return abs(ReconstructedParticle2Track::getRP2TRK_D0(Jet_Durham ,EFlowTrack_1))")
            #df = df.Define("RecoJetTrack_absZ0", "return abs(ReconstructedParticle2Track::getRP2TRK_Z0(Jet_Durham ,EFlowTrack_1))")
            
            #df = df.Define("RecoJetTrack_absD0sig", "return abs(ReconstructedParticle2Track::getRP2TRK_D0_sig(Jet_Durham ,EFlowTrack_1))") #significance
            #df = df.Define("RecoJetTrack_absZ0sig", "return abs(ReconstructedParticle2Track::getRP2TRK_Z0_sig(Jet_Durham ,EFlowTrack_1))")
            
            #df = df.Define("RecoJetTrack_D0cov", "ReconstructedParticle2Track::getRP2TRK_D0_cov(Jet_Durham ,EFlowTrack_1)") #variance (not sigma)
            #df = df.Define("RecoJetTrack_Z0cov", "ReconstructedParticle2Track::getRP2TRK_Z0_cov(Jet_Durham ,EFlowTrack_1)")
            
            # access the jet constituents:
            #df = df.Define("jetconstituents_ee_genkt", "JetConstituentsUtils::get_constituents(JetDurhamN2) ")
#            df = df.Define("jetconstituents", "JetConstituentsUtils::build_constituents_cluster( jetconstituents_ee_genkt ) ")
		
            #df = df.Define("n_RecoJets","JetConstituentsUtils::count_jets( jetconstituents_ee_genkt )") #count how many jets are in the event in total

            # access the "dmerge" distances:
            #df = df.Define("dmerge_23", "JetClusteringUtils::get_exclusive_dmerge( FCCAnalysesJets, 2)" )
...
########################################## 
            ...
            df = df.Define("Reco_ll_energy", "if (n_RecoMuons>1) return (RecoMuon_e.at(0) + RecoMuon_e.at(1)); else if (n_RecoElectrons>1) return (RecoElectron_e.at(0) + RecoElectron_e.at(1)); else return float(-1.);")
            df = df.Define("Reco_ll_px", "if (n_RecoMuons>1) return (RecoMuon_px.at(0) + RecoMuon_px.at(1)); else if (n_RecoElectrons>1) return (RecoElectron_px.at(0) + RecoElectron_px.at(1)); else return float(-1.);")
            df = df.Define("Reco_ll_py", "if (n_RecoMuons>1) return (RecoMuon_py.at(0) + RecoMuon_py.at(1)); else if (n_RecoElectrons>1) return (RecoElectron_py.at(0) + RecoElectron_py.at(1)); else return float(-1.);")
            df = df.Define("Reco_ll_pz", "if (n_RecoMuons>1) return (RecoMuon_pz.at(0) + RecoMuon_pz.at(1)); else if (n_RecoElectrons>1) return (RecoElectron_pz.at(0) + RecoElectron_pz.at(1)); else return float(-1.);")
            df = df.Define("Reco_ll_invMass", "if (n_RecoMuons>1) return sqrt(Reco_mumu_energy*Reco_mumu_energy - Reco_mumu_px*Reco_mumu_px - Reco_mumu_py*Reco_mumu_py - Reco_mumu_pz*Reco_mumu_pz ); else if (n_RecoElectrons>1) return sqrt(Reco_ee_energy*Reco_ee_energy - Reco_ee_px*Reco_ee_px - Reco_ee_py*Reco_ee_py - Reco_ee_pz*Reco_ee_pz ); else return float(-1.);")
